...
public sealed class PsiSectionAssemblerTests
{
    ...
    public void PushPacket_ParsesTwoSectionsFromSinglePayload()
    {
        ...
        Assert.That(ready[0].ToArray(), Is.EqualTo(sectionA));
        Assert.That(ready[1].ToArray(), Is.EqualTo(sectionB));
    }
    ...
    public void PushPacket_DropsInvalidSectionLengthAndResyncsFromNextPusi()
    {
        ...
        var invalidReady = assembler.PushPacket(invalidPacket);
        ...
        var validReady = assembler.PushPacket(resyncPacket);

        Assert.That(validReady, Has.Count.EqualTo(1));
        Assert.That(validReady[0].ToArray(), Is.EqualTo(validSection));
    }
